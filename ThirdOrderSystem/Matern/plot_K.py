...
for file, color in zip(file_list, colors):
    _, K, _ = np.load(os.path.join(folder_name, file), allow_pickle=True)
...
K_seq = np.array(K_seq)
sample_points_squared = np.array(sample_points_squared)
point_seq = np.array(point_seq)

# 设置线条宽度和标记大小
ax.plot(sample_points_squared, K_seq, color='ForestGreen', linestyle='-', linewidth=2, alpha=0.8)
ax.scatter(sample_points_squared, K_seq, color='ForestGreen', marker='P', s=100)


ax.set_xlabel(r'${1}/{N^{4}}$', fontsize=font_size)
ax.set_ylabel(r'$||\hat{K}^{(\infty)}-K^*||_F$', fontsize=font_size)
ax.tick_params(axis='x', labelsize=font_size)
ax.tick_params(axis='y', labelsize=font_size)

yticks_original = np.array([1, 2, 3 ]) * 1e-5
...
plt.tight_layout()
plt.savefig(f'./tos_Matern_4_K.pdf')
