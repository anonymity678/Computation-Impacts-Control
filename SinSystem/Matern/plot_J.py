...
for file, color in zip(file_list, colors):
    _, _, J = np.load(os.path.join(folder_name, file), allow_pickle=True)
...
yticks_original = np.array([3, 6, 9, 12]) * 1e-2
...
plt.tight_layout()
plt.savefig(f'./sin_Matern_4_J.pdf')
